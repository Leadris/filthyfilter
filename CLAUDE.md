...
- Projekt je čisté HTML/CSS/JS bez buildu. Nepridávaj framework ani npm závislosti do toho, čo sa nasádza, a nehostuj vlastný backend. Výnimka povolená 7. 9. 2026: `package.json` smie držať vývojové závislosti pre prehliadačové testy (`tests/`). Do webrootu nesmie ísť ani `package.json`, ani `node_modules`. **Od 6. 9. 2026 web volá existujúce `whispair-api`** (`D:\whispAir-IT\whispair-api`, PHP + PostgreSQL, `api.whispair.sk`) na odoslanie dopytu a atribúcie. Nestavaj druhé CRM ani vlastnú databázu; čo tá platforma už vie, je v `docs/MARKETING_PLAN.md`.
